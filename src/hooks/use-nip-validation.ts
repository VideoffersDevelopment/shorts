...
import { validateNipAction } from "@/app/actions/companies/validate-nip"
...
export interface UseNipValidationReturn {
...
export function useNipValidation(): UseNipValidationReturn {
  ...
  const validateNip = useCallback(async (nip: string) => {
    ...
    try {
      const result = await validateNipAction(trimmedNip)
