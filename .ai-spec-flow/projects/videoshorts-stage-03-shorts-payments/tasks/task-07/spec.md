...
| `src/lib/shorts/stats.ts` | Create | Stats tracking utility functions |
| `src/app/api/shorts/[id]/track/route.ts` | Create | Public stats tracking endpoint |
...
// - Track view (increment stats) - see section 13 below
// - Render PublicShortView component
```

### 13. Stats Tracking Implementation

**View Tracking (Server-Side in Page Component):**
```typescript
// In PublicShortPage server component, after fetching short:
...
// Implementation in src/lib/shorts/stats.ts (create if needed):
...
**CTA Click Tracking (Client-Side via API):**
```typescript
// ShortCtaButton component calls API on click:
const handleCtaClick = async () => {
  ...
  // Open CTA link immediately (don't wait)
  window.open(ctaLink, '_blank')
}

// API Route: POST /api/shorts/[id]/track
// Body: { event: 'cta_click' | 'like' | 'share' }
// Updates ShortStats accordingly:
// - cta_click -> increment ctaClicks
...
**Like Tracking (if implemented):**
```typescript
// Similar pattern - client calls POST /api/shorts/[id]/track
// with { event: 'like' }
// Consider requiring session for likes to prevent spam
```
