...
export async function POST(request: Request) {
  ...
  //    - Send "shorts/transcode.completed" Inngest event (triggers cleanup job)
  //    - **IMPORTANT: Send processing complete email to company owner**
  //      await sendProcessingCompleteEmail({
  //        to: companyOwner.email,
  //        shortTitle: short.title,
  //        shortId: short.id,
  //        publicUrl: `${process.env.NEXT_PUBLIC_APP_URL}/shorts/${short.id}`
  //      })
  // 5. On error:
...
//    - Fetch company profile with viesVerified field
//    - If !company.viesVerified:
//      return { error: "COMPANY_NOT_VERIFIED", requiresVerification: true }
//    - This check prevents unverified companies from publishing
// 4. CREDIT CHECK:
