...
The following files already exist and will be USED (not created):

| File | Status | Notes |
|------|--------|-------|
| `src/hooks/use-media-query.ts` | EXISTS | Used in FilterPanel for responsive design |
| `src/hooks/use-debounce.ts` | task-03 | Created in Task 03 |
| `src/app/api/tags/search/route.ts` | EXISTS | Used by TagFilter for autocomplete |
| `src/app/api/categories/[categoryId]/subcategories/route.ts` | EXISTS | Reference for categories API |
...
**Note:** Uses existing `/api/tags/search` endpoint from Stage 3 and `useDebounce` from Task 03.
...
import { useDebounce } from '@/hooks/use-debounce' // From Task 03
...
**Note:** Uses existing `useMediaQuery` hook from codebase.
...
import { useMediaQuery } from '@/hooks/use-media-query' // Already exists in codebase
import type { FeedFilters } from '@/lib/types/feed'
...
1. **useMediaQuery Hook:** Already exists at `src/hooks/use-media-query.ts` - use it directly.

2. **useDebounce Hook:** Created in Task 03 at `src/hooks/use-debounce.ts` - import from there.

3. **Tags Search API:** Already exists at `src/app/api/tags/search/route.ts` (Stage 3) - use it directly.

4. **Categories API:** Uses existing categories data. CategoryMultiSelect fetches from `/api/categories` endpoint.

5. **Geolocation Permission:** Handle gracefully if user denies permission.

6. **URL State:** All filters sync with URL for shareability and back button support.
